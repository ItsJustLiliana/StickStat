...
export function AppNav({name, photoPath, admin, teamAdmin, hasTeamAccess}:{name:string; photoPath:string|null; admin:boolean; teamAdmin:boolean; hasTeamAccess:boolean}) {
  ...
  useEffect(()=>{fetch("/api/favorite-teams").then(response=>response.json()).then(body=>setHasFavorites(Number(body.data?.count)>0)).catch(()=>{})},[]);
  ...
  const showTeamTabs=hasTeamAccess||hasFavorites;
  return <><header className="topbar">
    <Link ref={brandRef} className="brand" href="/dashboard"><span className="brand-mark">S</span>StickStat</Link>
    <nav className="topnav" aria-label="Hoofdnavigatie">{links.filter(link=>showTeamTabs||link.href==="/dashboard").map(link => <Link key={link.href} href={link.href} aria-current={isCurrent(link.href) ? "page" : undefined}>{link.label}</Link>)}</nav>
    <div className="account-actions" ref={actionsRef}><AndroidAppDownload/>{compact ? <div className="topbar-more" ref={moreRef}><button className="theme-toggle topbar-more-button" type="button" aria-label="Meer opties" aria-expanded={moreOpen} onClick={() => setMoreOpen(value => !value)}><Ellipsis size={21}/>{(showInbox||showNotifications)&&<i aria-hidden="true"/>}</button>{moreOpen && <div className="topbar-more-menu">{extraActions}</div>}</div> : extraActions}<ThemeToggle/><Link className="avatar" href="/profile" aria-current={isCurrent("/profile") ? "page" : undefined} title={`${name} - Profiel`}>{photoPath ? <Image unoptimized width={36} height={36} src={photoPath} alt=""/> : name[0]?.toUpperCase()}</Link></div>
  </header><nav className="mobile-nav" aria-label="Mobiele navigatie">{links.filter(link=>showTeamTabs||link.href==="/dashboard").map(({href, label, icon:Icon}) => <Link key={href} href={href} aria-current={isCurrent(href) ? "page" : undefined}><Icon size={19}/>{label}</Link>)}</nav></>;
}
