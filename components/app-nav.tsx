...
export function AppNav({name, photoPath, admin, teamAdmin}:{name:string; photoPath:string|null; admin:boolean; teamAdmin:boolean}) {
  const pathname = usePathname(), [hasDownload, setHasDownload] = useState(false), [joinCount, setJoinCount] = useState(0), [moreOpen, setMoreOpen] = useState(false), moreRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => { if (!teamAdmin) return; fetch("/api/team-join-requests").then(response => response.json()).then(body => setJoinCount(Number(body.data?.count) || 0)).catch(() => {}); }, [teamAdmin]);
  ...
  const showInbox = teamAdmin && joinCount > 0, overflow = (hasDownload ? 1 : 0) + (admin ? 1 : 0) + (showInbox ? 1 : 0) + 2 > 4;
  ...
  return <><header className="topbar">
    ...
    <div className="account-actions"><AndroidAppDownload/>{!overflow && platformAction}{!overflow && inboxAction}{overflow && <div className="topbar-more" ref={moreRef}><button className="theme-toggle" type="button" aria-label="Meer opties" aria-expanded={moreOpen} onClick={() => setMoreOpen(value => !value)}><Ellipsis size={21}/></button>{moreOpen && <div className="topbar-more-menu">{platformAction}{inboxAction}</div>}</div>}<NotificationsMenu/><ThemeToggle/><Link className="avatar" href="/profile" aria-current={isCurrent("/profile") ? "page" : undefined} title={`${name} · Profiel`}>{photoPath ? <Image unoptimized width={36} height={36} src={photoPath} alt=""/> : name[0]?.toUpperCase()}</Link></div>
