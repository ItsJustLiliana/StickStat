...
export function AppNav({name, photoPath, admin, teamAdmin}:{name:string; photoPath:string|null; admin:boolean; teamAdmin:boolean}) {
  ...
  const extraActions = <>{platformAction}{inboxAction}{notificationAction}<ThemeToggle/></>;
  return <><header className="topbar">
    ...
    <div className="account-actions" ref={actionsRef}><AndroidAppDownload/>{compact ? <div className="topbar-more" ref={moreRef}><button className="theme-toggle" type="button" aria-label="Meer opties" aria-expanded={moreOpen} onClick={() => setMoreOpen(value => !value)}><Ellipsis size={21}/></button>{moreOpen && <div className="topbar-more-menu">{extraActions}</div>}</div> : extraActions}<Link className="avatar" href="/profile" aria-current={isCurrent("/profile") ? "page" : undefined} title={`${name} · Profiel`}>{photoPath ? <Image unoptimized width={36} height={36} src={photoPath} alt=""/> : name[0]?.toUpperCase()}</Link></div>
