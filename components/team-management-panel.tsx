...
export function TeamManagementPanel({teamId,accounts,members,players}:{teamId:string;accounts:Account[];members:Member[];players:Player[]}){
  ...
  return <>
    ...
    <section className="card" style={{marginTop:18}}>
      <div className="card-head"><div><span className="eyebrow">Actieve selectie</span><h2>Spelersprofielen</h2></div><span className="badge">{players.length}</span></div>
      {players.length?players.map(player=><div className="sync-item" key={player.id}><div><strong>{player.displayName}</strong><div className="muted" style={{fontSize:12}}>{player.userId?"Gekoppeld aan een account":"Nog niet aan een account gekoppeld"}</div></div><button className="button secondary danger-button" disabled={busy} onClick={()=>{if(window.confirm(`Weet je zeker dat je ${player.displayName} uit de actieve selectie wilt verwijderen? Oude wedstrijdstatistieken blijven bewaard.`))void run(()=>request(`/api/teams/${teamId}/players`,"DELETE",{playerId:player.id}),"Speler uit de actieve selectie verwijderd.")}}>Verwijderen</button></div>):<p className="muted">Er zijn nog geen actieve spelersprofielen.</p>}
    </section>
