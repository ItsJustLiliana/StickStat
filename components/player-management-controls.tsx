...
type PlayerInput = { firstName: string; namePrefix: string | null; lastName: string; shirtNumber: number | null; position: string | null; trainingMember: boolean; matchMember: boolean; isSubstitute: boolean; staffRoles?: ("coach"|"trainer")[] };
...
export function PlayerCreateControl({ teamId }: { teamId: string }) {
  ...
  return <><button className="button compact-button" type="button" onClick={() => { setMessage(""); setOpen(true) }}><Plus size={16} /> Teamlid toevoegen</button>{open && <ManagementDialog title="Teamlid toevoegen" onClose={() => !busy && setOpen(false)}><p className="muted">Voeg een speler, coach of trainer toe. Een account kan later gekoppeld worden.</p><form className="form-stack" onSubmit={submit}><PlayerFields /><StaffRoleFields /><div className="member-actions"><button className="button" disabled={busy}>{busy ? "Toevoegen…" : "Teamlid toevoegen"}</button><button className="button secondary" type="button" disabled={busy} onClick={() => setOpen(false)}>Annuleren</button></div>{message && <p className="form-message" role="alert">{message}</p>}</form></ManagementDialog>}</>;
}
