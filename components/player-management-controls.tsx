...
export function PlayerDetailManagement({teamId,playerId,player,linkedAccount}:{teamId:string;playerId:string;player:PlayerInput&{displayName:string};linkedAccount:boolean}){
  ...
  async function remove(){setBusy(true);setMessage("");try{await request(`/api/teams/${teamId}/players`,"DELETE",{playerId});router.push(`/players?team=${teamId}`);router.refresh()}catch(error){setMessage(error instanceof Error?error.message:"Speler verwijderen mislukt");setBusy(false)}}
