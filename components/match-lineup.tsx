...
export function MatchLineup({ matchId, teamId, canEdit, players, initialFormation, initialPositions, initialSubstitutions, initialBench }: {
  ...
  useEffect(() => {
    if (!substitutionsDirty.current || substitutions.some(item => !item.playerInId || !item.playerOutId || item.playerInId === item.playerOutId)) return;
    substitutionsDirty.current = false;
    void saveSubstitutions();
  }, [substitutions]);
