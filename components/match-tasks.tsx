...
type Task = {id: string; taskType: MatchTaskType; player: {displayName: string} | null; user: {name: string} | null};
...
export function MatchTasks({matchId, teamId, canEdit, initialTasks, members}: {matchId: string; teamId: string; canEdit: boolean; initialTasks: Task[]; members: TeamMember[]}) {
  ...
  function add() {
    ...
    startTransition(async () => {
      try { await request("POST", {taskType, playerId: userId}); setDialogOpen(false); setUserId(""); router.refresh(); }
  ...
  return <section className="card match-tasks-card">
    ...
    {tasks.length === 0 ? <div className="empty">Nog geen taken aangewezen.</div> : <div className="match-task-sections">
      {matchTaskTypes.map(type => { const assignments = tasks.filter(task => task.taskType === type); if (!assignments.length) return null; return <section className="match-task-section" key={type} aria-labelledby={`match-task-${type}`}><h3 id={`match-task-${type}`}>{matchTaskLabels[type]}</h3><ul>{assignments.map(task => { const name = task.player?.displayName ?? task.user?.name ?? "Onbekend"; return <li key={task.id}><span>{name}</span>{canEdit && <button className="icon-button" type="button" aria-label={`${name} verwijderen bij ${matchTaskLabels[type]}`} title="Verwijderen" disabled={pending} onClick={() => void remove(task.id)}><X size={16}/></button>}</li>; })}</ul></section>; })}
    </div>}
