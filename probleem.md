...
- }
-
- .agenda-week-separator {
- margin-top: 14px
- }
- }
  ❯ tests/mobile-refresh-and-player-order.test.ts:12:17
  10|     expect(css).toMatch(/\.topbar\s*\{[\s\S]*?position:\s*fixed/);
  11|     expect(css).toMatch(/\.mobile-nav\s*\{[\s\S]*?position:\s*fixed/);
  12|     expect(css).toContain("overscroll-behavior-y: none");
  |                 ^
  13|   });
  14|
  ⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯[2/2]⎯
  ...
  Start at  17:48:21
  Duration  7.92s (transform 810ms, setup 0ms, collect 2.44s, tests 900ms, environment 18ms, prepare 6.45s)
