...
const mocks = vi.hoisted(() => ({authorize: vi.fn(), member: vi.fn(), create: vi.fn(), remove: vi.fn()}));
...
vi.mock("@/lib/db", () => ({db: {
  ...
  teamMembership: {findUnique: mocks.member},
  matchTask: {create: mocks.create, deleteMany: mocks.remove},
}}));

import {DELETE, POST} from "../app/api/matches/[matchId]/tasks/route";

const teamId = "cm00000000000000000000001", userId = "cm00000000000000000000003", taskId = "cm00000000000000000000004";
const params = {params: Promise.resolve({matchId: "match"})};
const post = () => POST(new Request("http://localhost/api/tasks", {method: "POST", body: JSON.stringify({teamId, userId, taskType: "driving"})}), params);
...
describe("wedstrijdtaken API", () => {
  it("laat alleen teambeheerders een taak voor een eigen teamlid toevoegen", async () => {
    expect((await post()).status).toBe(200);
    expect(mocks.authorize).toHaveBeenCalledWith(teamId);
    expect(mocks.member).toHaveBeenCalledWith({where: {userId_teamId: {userId, teamId}}, select: {id: true}});
    expect(mocks.create).toHaveBeenCalledWith(expect.objectContaining({data: {matchId: "match", teamId, userId, taskType: "driving"}}));
  });
  it("weigert een niet-teamlid", async () => {
