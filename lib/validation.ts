...
export const playerSchema = z.object({ firstName: z.string().trim().min(1).max(80), namePrefix: z.string().trim().max(30).nullable().optional(), lastName: z.string().trim().min(1).max(80), shirtNumber: z.number().int().min(0).max(999).nullable().optional(), position: z.string().trim().max(80).nullable().optional(), active: z.boolean().default(true), trainingMember:z.boolean().default(true), matchMember:z.boolean().default(true), isSubstitute:z.boolean().default(false),staffRoles:z.array(z.enum(["coach","trainer"])).default([]) });
