...
}).refine(value=>Boolean(value.username||value.email),{message:"Gebruikersnaam is verplicht",path:["username"]}).refine(value=>Boolean(value.inviteToken||(value.clubId&&value.teamId)),{message:"Kies een club en team",path:["teamId"]}).refine((value)=>value.password===value.confirmPassword,{message:"Wachtwoorden komen niet overeen",path:["confirmPassword"]}).transform(value=>({...value,username:value.username??value.email!.split("@")[0].toLowerCase()}));
export const playerSchema = z.object({ firstName: z.string().trim().min(1).max(80), namePrefix: z.string().trim().max(30).nullable().optional(), lastName: z.string().trim().min(1).max(80), shirtNumber: z.number().int().min(0).max(999).nullable().optional(), position: z.string().trim().max(80).nullable().optional(), active: z.boolean().default(true), trainingMember:z.boolean().default(true), matchMember:z.boolean().default(true), isSubstitute:z.boolean().default(false) });
export const eventSchema = z.object({ playerId: z.string().cuid().nullable().optional(), relatedPlayerId: z.string().cuid().nullable().optional(), minute: z.number().int().min(0).max(120).nullable().optional(), type: z.enum(["mvp","goal","assist","green_card","yellow_card","red_card","substitution","save","penalty_corner","penalty_stroke","custom"]), notes: z.string().trim().max(1000).nullable().optional() });
