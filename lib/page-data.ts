...
export async function pageContext(requested?:string){
  ...
  const [teams,player,favorites]=await Promise.all([
    ...
    db.favoriteTeam.findMany({where:{userId:user.id},select:{teamId:true}}),
  ]);
  ...
  return {user,teams,team,favoriteTeamIds:favorites.map(favorite=>favorite.teamId)};
}
