...
async function main(){
  ...
  const user=await db.user.upsert({where:{email},update:{name:"StickStat beheerder",platformRole:"admin"},create:{name:"StickStat beheerder",email,passwordHash,platformRole:"admin"}});
