...
export default async function MatchDetail({ params, searchParams }: { params: Promise<{ matchId: string }>; searchParams: Promise<{ team?: string }> }) {
  ...
  const match = await db.match.findUnique({ where: { id: matchId }, include: { homeTeam: { include: { club: true } }, awayTeam: { include: { club: true } }, season: true, playerStats: { include: { player: true } }, events: { include: { player: true, relatedPlayer: true }, orderBy: { createdAt: "asc" } }, attendance: true, plans: true, tasks: {include: {user: {select: {id: true, name: true}}}, orderBy: {createdAt: "asc"}} } });
  if (!match) notFound();
