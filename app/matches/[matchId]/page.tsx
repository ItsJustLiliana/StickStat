...
export default async function MatchDetail({params,searchParams}:{params:Promise<{matchId:string}>;searchParams:Promise<{team?:string}>}){
  ...
  return <PageShell user={user}><div className="page-head"><div><span className="eyebrow">Wedstrijddetail</span><h1>{match.status==="finished"?"Eindstand":"Wedstrijd"}</h1></div><span className="badge">{match.status}</span></div><section className="hero" style={{gridTemplateColumns:"1fr",textAlign:"center"}}><div className="match-scoreboard"><div className="match-score-team"><ClubLogo name={match.homeTeam.club.name} path={match.homeTeam.club.logoLocalPath??match.homeTeam.club.logoUrl}/><strong>{match.homeTeam.shortName}</strong></div><div className="rank-number mono match-score">{match.homeScore??"–"} <span style={{color:"white"}}>–</span> {match.awayScore??"–"}</div><div className="match-score-team"><ClubLogo name={match.awayTeam.club.name} path={match.awayTeam.club.logoLocalPath??match.awayTeam.club.logoUrl}/><strong>{match.awayTeam.shortName}</strong></div></div></section><section className="metrics match-metrics"><div className="metric"><span>Datum</span><strong style={{fontSize:17}}>{match.date.toLocaleDateString("nl-NL",{day:"numeric",month:"long",year:"numeric"})}</strong></div><div className="metric"><span>Tijd</span><strong style={{fontSize:17}}>{match.startTime??"–"}</strong></div><div className="metric"><span>Locatie</span><strong style={{fontSize:17}}>{match.venue??"–"}</strong></div><div className="metric"><span>Competitie</span><strong style={{fontSize:17}}>{match.competition??"–"}</strong></div></section>{ownTeam&&<MatchTeamStatsForm matchId={match.id} teamId={ownTeam.id} teamScore={ownScore} initialRows={initialRows} canEdit={canEdit}/>}</PageShell>;
}
