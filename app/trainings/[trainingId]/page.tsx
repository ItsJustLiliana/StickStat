...
export default async function TrainingDetail({ params, searchParams }: { params: Promise<{ trainingId: string }>; searchParams: Promise<{ team?: string }> }) {
  ...
  const players = await db.player.findMany({ where: { teamId: training.teamId, active: true, OR: [{ trainingMember: true }, { matchMember: true, trainingMember: false }, { trainingAttendance: { some: { trainingId: training.id } } }] }, include: { user: { select: { photoPath: true } } }, orderBy: [{ lastName: "asc" }, { namePrefix: "asc" }, { firstName: "asc" }] }), attendanceByPlayer = new Map(training.attendance.map(item => [item.playerId, item]));
