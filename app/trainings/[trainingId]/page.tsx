...
export default async function TrainingDetail({ params, searchParams }: { params: Promise<{ trainingId: string }>; searchParams: Promise<{ team?: string }> }) {
  ...
  const players = await db.player.findMany({ where: { teamId: training.teamId, OR: [{ active: true, trainingMember: true }, { active: true, matchMember: true, trainingMember: false }, { trainingAttendance: { some: { trainingId: training.id } } }] }, include: { user: { select: { photoPath: true } } }, orderBy: [{ lastName: "asc" }, { namePrefix: "asc" }, { firstName: "asc" }] }), status = new Map(training.attendance.map(item => [item.playerId, item.status]));
  const people = players.map(player => ({ playerId: player.id, name: player.displayName, photoPath: player.user?.photoPath ?? player.photoPath, status: status.get(player.id) ?? "unknown" as const, editable: canManage || player.userId === user.id, isSubstitute: player.isSubstitute, isSupplemental: player.matchMember && !player.trainingMember }));
