...
export async function POST(request:NextRequest){
  try{
    ...
    try { rateLimit(`register:${ip}`,5,10*60_000); }
    catch(error) { const minutes=Math.max(1,Math.ceil((error as RateLimitError).retryAfterMs/60_000)); throw new HttpError(429,"RATE_LIMITED",`Te veel registratiepogingen. Probeer over ${minutes} minuut${minutes===1?"":"en"} opnieuw.`); }
