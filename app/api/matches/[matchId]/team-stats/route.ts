...
const row = z.object({ playerId: z.string().cuid(), participation: z.enum(["absent", "substitute", "starter"]), goals: z.number().int().min(0).max(20), assists: z.number().int().min(0).max(20), saves: z.number().int().min(0).max(100), greenCards: z.number().int().min(0).max(3), yellowCards: z.number().int().min(0).max(3), redCards: z.number().int().min(0).max(3), mvp: z.boolean(), notes: z.string().trim().max(500) });
  ...
  try {
    ...
    await db.$transaction(async transaction => {
      ...
      for (const item of input.rows) {
        ...
        else await transaction.playerMatchStats.upsert({ where: { matchId_playerId: { matchId, playerId: item.playerId } }, update: { started: item.participation === "starter", minutesPlayed: null, goals: item.goals, assists: item.assists, saves: item.saves, mvp: item.mvp, notes: item.notes || null }, create: { matchId, playerId: item.playerId, started: item.participation === "starter", goals: item.goals, assists: item.assists, saves: item.saves, mvp: item.mvp, notes: item.notes || null } });
