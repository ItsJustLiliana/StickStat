    ...
    const [matches,players]=await Promise.all([
      ...
      db.player.findMany({
        ...
        include:{
          ...
          relatedEvents:{where:{match:matchFilter}},
        },
      }),
    ]);
    return ok({
      ...
      players:players.map(player=>({
        ...playerTotals(player.id,player.matchStats,[...player.events,...player.relatedEvents]),
        id:player.id,
        name:player.displayName,
        mvp:playerTotals(player.id,player.matchStats,[...player.events,...player.relatedEvents]).mvps,
