...
export async function DELETE(request:Request,{params}:{params:Promise<{teamId:string}>}){
  try{
    const {teamId}=await params;await authorizeTeamAdmin(teamId);const {playerId}=playerIdSchema.parse(await request.json()),player=await db.player.findUnique({where:{id:playerId},select:{id:true,teamId:true,userId:true}});
    if(!player||player.teamId!==teamId)throw new HttpError(404,"PLAYER_NOT_FOUND","Speler niet gevonden");
