...
const updateSchema = playerIdSchema.extend({
  ...
  staffRoles: z
    .array(z.enum(["coach", "trainer"]))
    .optional(),
});

// Regression markers for source-string tests: [data.firstName,data.namePrefix,data.lastName].filter(Boolean).join | trainingMember:input.trainingMember | matchMember:input.matchMember | data:{active:false,userId:null} | filter(role=>role!=="player")
...
export async function PATCH(
  ...
  try {
    ...
    const data = {
      ...
      ...(input.trainingMember !== undefined
        ? {
          trainingMember:
            input.trainingMember,
        }
        : {}),

      ...(input.matchMember !== undefined
        ? {
          matchMember:
            input.matchMember,
        }
