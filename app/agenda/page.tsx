...
export default async function Agenda({searchParams}:{searchParams:Promise<{team?:string}>}){
 ...
 const items=[...matches.map(match=>({id:match.id,type:"match" as const,date:match.date,time:match.startTime,title:`${match.homeTeam.shortName} – ${match.awayTeam.shortName}`,venue:match.venue,href:`/matches/${match.id}?team=${team.id}`,attendance:match.attendance})),...trainings.map(training=>({id:training.id,type:"training" as const,date:training.date,time:training.startTime,title:training.title,venue:training.venue,href:`/trainings/${training.id}?team=${team.id}`,attendance:training.attendance}))].sort((a,b)=>a.date.getTime()-b.date.getTime());
 return <PageShell user={user}><div className="page-head"><div><span className="eyebrow">Wedstrijden & trainingen</span><h1>Agenda</h1></div><TeamSelector teams={teams} current={team.id}/></div>{canManage&&<TrainingCreateForm teamId={team.id}/>}<section className="card agenda-card"><div className="card-head"><h2>Teamagenda</h2><span className="badge">{items.length} afspraken</span></div><div className="agenda-list">{items.map(item=>{const present=item.attendance.filter(row=>row.status==="present").length,absent=item.attendance.filter(row=>row.status==="absent").length;return <Link className="agenda-row" href={item.href} key={`${item.type}-${item.id}`}><time><strong>{item.date.toLocaleDateString("nl-NL",{day:"2-digit",month:"short"})}</strong><span>{item.time??"–"}</span></time><span><small className={`agenda-type ${item.type}`}>{item.type==="match"?"Wedstrijd":"Training"}</small><strong>{item.title}</strong><small>{item.venue??"Locatie onbekend"}</small></span><span className="attendance-summary"><i className="present">{present}</i><i className="absent">{absent}</i></span></Link>})}</div>{!items.length&&<div className="empty">Nog niets in de agenda.</div>}</section></PageShell>;
}
