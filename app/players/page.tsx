...
export default async function Players({ searchParams }: { searchParams: Promise<{ team?: string }> }) {
  ...
  const unlinkedPlayers = players.filter(player => !linkedPlayerIds.has(player.id) && player.staffRoles.length===0);
