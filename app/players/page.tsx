...
export default async function Players({searchParams}:{searchParams:Promise<{team?:string}>}){
  ...
  function memberPerson(membership:(typeof memberships)[number],role:TeamRole|"unassigned"):RosterPerson{
    ...
    return {key:`${role}-${membership.userId}`,name:player?.displayName??membership.user.name,sortName:player?.lastName??accountLastName(membership.user.name),photoPath:membership.user.photoPath??player?.photoPath??null,subtitle:player?.position??(role==="player"?"Nog niet aan een spelersprofiel gekoppeld":membership.user.email),href:`/team-members/${membership.userId}?team=${team.id}`,shirtNumber:role==="player"?player?.shirtNumber:undefined,matches:role==="player"&&player?player.matchStats.length:undefined,goals,assists};
  }
  ...
  return <PageShell user={user}><div className="page-head"><div><span className="eyebrow">Selectie & staf</span><h1>Teamleden</h1></div><TeamSelector teams={teams} current={team.id}/></div><div className="role-sections">{sections.map(section=>{const people=memberships.filter(membership=>membership.roles.includes(section.role)&&(section.role!=="player"||membership.user.player?.teamId===team.id)).map(membership=>memberPerson(membership,section.role));if(section.role==="player")people.push(...unlinkedPlayers);sortByLastName(people);if(!people.length)return null;return <section key={section.role}><div className="role-section-head"><div><span className="eyebrow">{section.description}</span><h2>{section.title}</h2></div><span className="badge">{people.length}</span></div><div className="roster-list">{people.map(person=><RosterListItem person={person} key={person.key}/>)}</div></section>})}{unlinkedAccounts.length>0&&<section><div className="role-section-head"><div><span className="eyebrow">Geregistreerde accounts zonder spelersprofiel</span><h2>Nog niet aan een speler gekoppeld</h2></div><span className="badge">{unlinkedAccounts.length}</span></div><div className="roster-list">{unlinkedAccounts.map(person=><RosterListItem person={person} key={person.key}/>)}</div></section>}</div></PageShell>;
}
