...
export default async function Players({searchParams}:{searchParams:Promise<{team?:string}>}){
  ...
  return <PageShell user={user}><div className="page-head"><div><span className="eyebrow">Selectie & staf</span><h1>Teamleden</h1></div><TeamSelector teams={teams} current={team.id}/></div><div className="role-sections">{sections.map(section=>{const people=memberships.filter(membership=>membership.roles.includes(section.role)).map(membership=>memberPerson(membership,section.role));if(section.role==="player")people.push(...unlinkedPlayers);return <section key={section.role}><div className="role-section-head"><div><span className="eyebrow">{section.description}</span><h2>{section.title}</h2></div><span className="badge">{people.length}</span></div>{people.length?<div className="roster-list">{people.map(person=><RosterListItem person={person} key={person.key}/>)}</div>:<div className="card empty">Nog niemand met deze rol.</div>}</section>})}</div></PageShell>;
}
