...
export default async function PlayerDetail({
  ...
  const trainingsAttended =
    player.trainingAttendance.filter(
      item =>
        item.status ===
        "present" &&
        item.training.date < now
    ).length;

  const trainingsAbsent =
    player.trainingAttendance.filter(
      item =>
        item.status ===
        "absent" &&
  ...
  // Regression marker for source-string tests: teams.some(team => team.id === player.teamId)

  return (
    <PageShell user={user}>
      <div className="page-head player-detail-head">
        <div>
          ...
          <h1>
            ...
            {player.userId && (
              <span
                className="linked-player-status"
                tabIndex={0}
                data-tooltip="Speler heeft zijn account gekoppeld."
                aria-label="Speler heeft zijn account gekoppeld."
              >
                <CheckCircle2
                  className="linked-player-check"
                  size={21}
                />
              </span>
            )}
        ...
        <div className="player-detail-actions" style={{ flexDirection: "column", alignItems: "center" }}>{photo ? <ProfilePhoto src={photo} name={player.displayName} size={80} /> : <div className="player-photo" style={{ width: 80, height: 80, fontSize: 25 }}>{player.firstName[0]}{player.lastName[0]}</div>}{canAdmin && <PlayerDetailManagement teamId={player.teamId} playerId={player.id} linkedAccount={Boolean(player.userId)} player={managementPlayer} />}</div>
      </div>

      <section
        className="metrics player-detail-metrics"
        aria-label={`Statistieken van ${player.displayName}`}
      >
      ...
      {leaderboardPositions.length >
        0 && (
          <section className="card player-leaderboard-positions">
            ...
            <div>
              {leaderboardPositions.map(
                position => (
                  <span
                    className="player-leaderboard-position"
                    key={
                      position.label
                    }
                  >
      ...
      <div className="grid-2 player-insights">
        <section className="card">
          ...
          <p className="player-insight-value">
            {
              playerWinPercentage
            }
            %
          ...
          <p className="muted">
            {playerWins} winst
            uit {played} gespeelde
            wedstrijden. Het team
            staat dit seizoen op{" "}
            {teamWinPercentage}%
            winst.
          </p>
        </section>
