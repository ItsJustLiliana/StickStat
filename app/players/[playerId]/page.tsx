...
export default async function PlayerDetail({params,searchParams}:{params:Promise<{playerId:string}>;searchParams:Promise<{team?:string}>}){
  ...
  const player=await db.player.findUnique({where:{id:playerId},include:{user:true,team:{include:{club:true}},matchStats:{where:{match:{status:"finished"}},include:{match:true}},events:{where:{match:{status:"finished"}}}}});
  ...
  return <PageShell user={user}><div className="page-head"><div><span className="eyebrow">{player.team.club.name} · {player.team.name}</span><h1>{player.displayName}</h1><p className="muted">#{player.shirtNumber??"–"} · {player.position??"Positie onbekend"}</p></div><div className="player-detail-actions">{photo?<img className="player-photo image" style={{width:80,height:80}} src={photo} alt={`Profielfoto van ${player.displayName}`}/>:<div className="player-photo" style={{width:80,height:80,fontSize:25}}>{player.firstName[0]}{player.lastName[0]}</div>}{canAdmin&&<PlayerDetailManagement teamId={player.teamId} playerId={player.id} linkedAccount={Boolean(player.userId)} player={{firstName:player.firstName,namePrefix:player.namePrefix,lastName:player.lastName,displayName:player.displayName,shirtNumber:player.shirtNumber,position:player.position,trainingMember:player.trainingMember,matchMember:player.matchMember}}/>}</div></div><section className="metrics"><div className="metric"><span>Wedstrijden</span><strong>{played}</strong></div><div className="metric"><span>Goals</span><strong>{goals}</strong></div><div className="metric"><span>Assists</span><strong>{assists}</strong></div><div className="metric"><span>G + A</span><strong>{goals+assists}</strong></div><div className="metric"><span>Kaarten</span><strong>{cards}</strong></div><div className="metric"><span>MVP&apos;s</span><strong>{mvps}</strong></div></section><StatsCharts matches={chart} players={[{name:player.displayName,goals,assists}]}/><section className="card" style={{marginTop:18}}><h2>Gemiddelden</h2><p className="muted">{played?(goals/played).toFixed(2):"0.00"} goals per wedstrijd · {played?(assists/played).toFixed(2):"0.00"} assists per wedstrijd</p></section></PageShell>;
}
