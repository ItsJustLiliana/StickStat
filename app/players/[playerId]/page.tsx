...
export default async function PlayerDetail({params,searchParams}:{params:Promise<{playerId:string}>;searchParams:Promise<{team?:string}>}){
  const [{playerId},query]=await Promise.all([params,searchParams]),{user}=await pageContext(query.team);
  const player=await db.player.findUnique({where:{id:playerId},include:{user:true,team:{include:{club:true}},matchStats:{include:{match:true}},events:true}});
  if(!player)notFound();
