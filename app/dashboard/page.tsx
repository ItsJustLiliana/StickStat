...
export default async function Dashboard({searchParams}:{searchParams:Promise<{team?:string}>}){
  ...
  return <PageShell user={user}>
    ...
    <div className="grid-2">
      <section className="card"><div className="card-head"><h2>Laatste wedstrijden</h2><Link className="link" href={`/matches?team=${team.id}`}>Alles bekijken →</Link></div>{finished.slice(0,5).map(match=><Link href={`/matches/${match.id}?team=${team.id}`} className="match-row" style={{color:"inherit",textDecoration:"none"}} key={match.id}><div className="match-date">{match.date.toLocaleDateString("nl-NL",{day:"2-digit",month:"short"})}</div><div className="teams"><MatchTeamLabel name={match.homeTeam.shortName} own={match.homeTeamId===team.id} side="home"/><span className="match-versus">–</span><MatchTeamLabel name={match.awayTeam.shortName} own={match.awayTeamId===team.id} side="away"/></div><div className="score mono">{match.homeScore}–{match.awayScore}</div></Link>)}{!finished.length&&<div className="empty">Nog geen uitslagen beschikbaar.</div>}</section>
