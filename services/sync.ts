...
export async function syncTeam(teamId:string){
  ...
  try {
    const [matches,standings]=await Promise.all([provider.getMatches(team.externalIdentifier),provider.getStandings(team.externalIdentifier)]); let created=0,updated=0;
